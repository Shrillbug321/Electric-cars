...
function Delete() {
  ...
  const handleDelete = async (chargerName) => {
    try {
      ...
      // Pobierz zaktualizowane dane po usunięciu i zaktualizuj stan
  ...
  if (readError) {
    view = <div className="ui negative message">{readError.message}</div>;
  } else if (!readLoading) {
      ...
      <ul className="cards__items_small">
        {chargers.slice(startIndex, startIndex + itemsPerPage).map((city, index) => (
          <CardCityItem
            ...
            handleCityClick={(e) => handleDelete(city.props.children.props.children)}
            isStartCity
          />
        ))}
      </ul>
